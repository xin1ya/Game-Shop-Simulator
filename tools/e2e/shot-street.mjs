...
// 高机位大视野：全街（x ±22、z 4~17 尽收眼底）
await page.evaluate(()=>{
  window.BGS.managerPos.x = 0; window.BGS.managerPos.z = 10.5;
  const cam = window.BGS.camera; const a = window.innerWidth/window.innerHeight;
  cam.top=7.5; cam.bottom=-7.5; cam.left=-7.5*a*1.9; cam.right=7.5*a*1.9; cam.updateProjectionMatrix();
});
await new Promise(r=>setTimeout(r,600));
await page.screenshot({ path: path.join(SHOTS,'street_sidewalk.png') });
await browser.close(); server.close();
console.log('[shot-street] done');
