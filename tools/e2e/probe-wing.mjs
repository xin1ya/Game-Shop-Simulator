...
await page.evaluate(async ()=>{
  ...
  if (!E.buyExpansion(window.BGS.gs, 'wing_right')) throw new Error('buyExpansion failed');
...
// 第一人称站翼房南侧（9.4, 1.4）朝北：两张翼房桌（8.6/10.2, -0.6）应并排在前方
const setPose = ()=>page.evaluate(()=>{
  const fp = window.BGS.fp;
  fp.x = 9.4; fp.z = 1.4; fp.y = 0; fp.velY = 0;
  fp.yaw = 0.05; fp.pitch = -0.12;
});
for (let i=0;i<7;i++){ await setPose(); await new Promise(r=>setTimeout(r,90)); }
await page.screenshot({ path: path.join(SHOTS,'final_wing_fp.png') });
await browser.close(); server.close();
console.log('[probe-wing] done');
